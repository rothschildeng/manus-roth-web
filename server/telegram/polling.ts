...
function shopKeyboard(baseUrl: string): InlineKeyboard {
  ...
  return { inline_keyboard: [[{ text: "Open ROTH DIGITAL", url: `${baseUrl}/` }, { text: "Fund wallet", url: `${baseUrl}/wallet#deposit` }], ...categoryButtons, [{ text: "My order desk", url: `${baseUrl}/account` }, { text: "Support", callback_data: "shop:support" }]] };
}
...
export function shopHelpMessage() {
  return `<b>ROTH DIGITAL shop help</b>\n\n/catalog — open live catalog routes\n/fund — open wallet funding\n/support — open the support desk\n/order — learn where to view your private order status\n\nFor privacy, order status is available only in the signed-in website account or from an existing confirmation link.`;
}

export function shopOrderPrivacyMessage() {
  return "For privacy, this bot does not look up order status from an ID sent in chat. Sign in to the website account desk or open your existing confirmation link to view the current order record.";
}
...
async function handleShopUpdate(update: TelegramUpdate) {
  ...
  if (text.startsWith("/order") || callback?.data === "shop:order") { await sendTelegramMessage("shop", chatId, shopOrderPrivacyMessage(), { inline_keyboard: [[{ text: "Open my account", url: `${baseUrl}/account` }], [{ text: "Support", url: `${baseUrl}/support` }]] }); return; }
  ...
  if (text === "/help") { await sendTelegramMessage("shop", chatId, shopHelpMessage(), { inline_keyboard: [[{ text: "Open ROTH DIGITAL", url: `${baseUrl}/` }, { text: "Support", url: `${baseUrl}/support` }]] }); return; }
  ...
  await sendTelegramMessage("shop", chatId, shopHelpMessage());
}
...
async function sendPendingOrders(chatId: number) {
  ...
  await sendTelegramMessage("admin", chatId, `<b>ROTH REVIEW DESK</b>\n${pending.length} confirmed order(s) waiting for a manual decision.${pending.length > 12 ? " Showing the first 12; use the web admin panel for the complete queue." : ""}`);
...
async function sendPendingDeposits(chatId: number) {
  ...
  await sendTelegramMessage("admin", chatId, `<b>ROTH WALLET DESK</b>\n${pending.length} deposit request(s) waiting for a manual decision.${pending.length > 12 ? " Showing the first 12; use the web admin panel for the complete queue." : ""}`);
...
async function handleAdminUpdate(update: TelegramUpdate) {
  ...
  if (text === "/help") { await sendTelegramMessage("admin", chatId, "<b>ROTH ADMIN BOT</b>\n\n/orders — confirmed product orders waiting for decision\n/deposits — wallet deposits waiting for credit review\n/menu — open the current order queue\n\nApprove only after independent verification. Product delivery remains owner-controlled and manual."); return; }
  const orderMatch = callback?.data?.match(/^admin:(approve|reject):([a-zA-Z0-9]{8,64})$/);
  if (orderMatch) { const [, action, orderId] = orderMatch; const order = await getPaymentOrder(orderId!); if (!order || order.status !== "pending_admin" || order.purpose === "wallet_deposit") { await sendTelegramMessage("admin", chatId, order?.purpose === "wallet_deposit" ? "Crypto wallet funding must be credited from the wallet deposit review queue." : "This order is no longer waiting for manual approval."); return; } const next = action === "approve" ? "approved" : "rejected"; await updatePaymentOrder(orderId!, { status: next }); await recordOrderNotification(orderId!, order.customerId, next); await sendTelegramMessage("admin", chatId, `Order <b>${orderId}</b> was ${next}. ${action === "approve" ? "Send fulfilment manually through your approved owner process." : "No delivery action was triggered."}`); return; }
  ...
  await sendTelegramMessage("admin", chatId, "Use /orders for confirmed payment orders, /deposits for wallet deposits, or /help for the command guide.");
}
